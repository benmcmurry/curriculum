...
<table>
    <tr>
        ...
        <td>
            <h1>".$course['level_short_name']." ".$course['course_name']."</h1>
        </td>
    </tr>
    <tr>
        <td>
...
<article>
    ...
    $queryRequiredLearningExperiences = $elc_db->prepare("select *, Learning_experiences.name, Learning_experiences.learning_experience_id 
		...
		while($le = $resultLe->fetch_assoc()){
			...
			$html .="<li><a class='le_link' href='learning_experience.php?id=".$le['learning_experience_id']."'>".$le['name']."</a>. ".$le['short_description']."</li>";
		}            
        $html .="</ol>
